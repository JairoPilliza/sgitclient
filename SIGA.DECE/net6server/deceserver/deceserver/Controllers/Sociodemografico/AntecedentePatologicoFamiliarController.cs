...
namespace deceserver.Controllers.Sociodemografico
{
    ...
    [ApiController]
    public class AntecedentePatologicoFamiliarController : ControllerBase
    {
        private readonly JwtResource _JwtSRC = new();
        private readonly DeceSDAntecedentePatologicoFamiliarDao _ep = new();
        ...
        [HttpPost("Post")]
        public async Task<IActionResult> PostAsync([FromBody] DeceSDAntecedentePatologicoFamilia jsonData)
        {
            string token = HttpContext.Request.Headers.Single(x => x.Key == "Authorization").Value;
        ...
        [HttpPut("Put")]
        public async Task<IActionResult> PutAsync([FromQuery] string json, [FromBody] DeceSDAntecedentePatologicoFamilia jsonData)
        {
            string token = HttpContext.Request.Headers.Single(x => x.Key == "Authorization").Value;
