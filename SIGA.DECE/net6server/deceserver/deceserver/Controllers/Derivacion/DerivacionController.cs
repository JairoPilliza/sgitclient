...
namespace deceserver.Controllers.Derivacion
{
    ...
    public class DerivacionController : Controller
    {
        ...
        private readonly DeceDerivacionDao _dd= new();
        ...
        public async Task<IActionResult> GetAsync([FromQuery] string json)
        {
            ...
            _result = await _dd.GetAsync(authClientJson, json);
        ...
        public async Task<IActionResult> GetByIdAsync([FromQuery] string json)
        {
            ...
            _result = await _dd.GetByIdAsync(authClientJson, json);

            return Ok(_result);
        }

        [HttpPost("Post")]
        public async Task<IActionResult> PostAsync([FromBody] DeceDerivacion jsonData)
        {
            string token = HttpContext.Request.Headers.Single(x => x.Key == "Authorization").Value;

            string authClientJson = JsonConvert.SerializeObject(_JwtSRC.GetClaimJsonJwt<AuthModel>(token, "xjson"));

            _result = await _dd.PostAsync(authClientJson, JsonConvert.SerializeObject(jsonData));

            return Ok(_result);
        }

        [HttpPut("Put")]
        public async Task<IActionResult> PutAsync([FromQuery] string json, [FromBody] DeceDerivacion jsonData)
        {
            string token = HttpContext.Request.Headers.Single(x => x.Key == "Authorization").Value;

            string authClientJson = JsonConvert.SerializeObject(_JwtSRC.GetClaimJsonJwt<AuthModel>(token, "xjson"));

            _result = await _dd.PutAsync(authClientJson, json, JsonConvert.SerializeObject(jsonData));
        ...
        public async Task<IActionResult> DeleteAsync([FromQuery] string json)
        {
            ...
            _result = await _dd.DeleteAsync(authClientJson, json);
