...
namespace deceserver.Controllers.DeteccionRemisionCaso
{
    ...
    [ApiController]
    public class DeteccionRemisionCasoController : ControllerBase
    {
        private readonly JwtResource _JwtSRC = new();
        private readonly DeceDeteccionRemisionCasoDao _drc = new();
        ...
        public async Task<IActionResult> GetAsync([FromQuery] string json)
        {
            ...
            string authClientJson = JsonConvert.SerializeObject(_JwtSRC.GetClaimJsonJwt<AuthModel>(token, "xjson"));
             ...
             _result = await _drc.GetAsync(authClientJson, json);
        ...
        public async Task<IActionResult> GetByIdAsync([FromQuery] string json)
        {
            ...
            string authClientJson = JsonConvert.SerializeObject(_JwtSRC.GetClaimJsonJwt<AuthModel>(token, "xjson"));
             ...
             _result = await _drc.GetByIdAsync(authClientJson, json);

            return Ok(_result);
        }

        [HttpPost("Post")]
        public async Task<IActionResult> Post([FromBody] DeceDeteccionRemisionCaso jsonData)
        {
            string token = HttpContext.Request.Headers.Single(x => x.Key == "Authorization").Value;

            string authClientJson = JsonConvert.SerializeObject(_JwtSRC.GetClaimJsonJwt<AuthModel>(token, "xjson"));

             _result = await _drc.PostAsync(authClientJson, JsonConvert.SerializeObject(jsonData));

            return Ok(_result);
        }

        [HttpPut("Put")]
        public async Task<IActionResult> PutAsync([FromQuery] string json, [FromBody] DeceDeteccionRemisionCaso jsonData)
        {
            string token = HttpContext.Request.Headers.Single(x => x.Key == "Authorization").Value;

            string authClientJson = JsonConvert.SerializeObject(_JwtSRC.GetClaimJsonJwt<AuthModel>(token, "xjson"));

             _result = await _drc.PutAsync(authClientJson, json, JsonConvert.SerializeObject(jsonData));
        ...
        public async Task<IActionResult> DeleteAsync([FromQuery] string json)
        {
            ...
            string authClientJson = JsonConvert.SerializeObject(_JwtSRC.GetClaimJsonJwt<AuthModel>(token, "xjson"));
             ...
             _result = await _drc.DeleteAsync(authClientJson, json);
