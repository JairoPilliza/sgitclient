...
{
    public class DeceSDAntecedenteDificultadEscolarDao : ConnectionDB
    {
        ...
        public async Task<StructureResponse> GetAsync(string authClientJson, string identifierFormJson)
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("uspDeceSDAntecedenteDificultadEscolarOpcionListar", con))
                {
